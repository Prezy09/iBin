...
function settings_clamp_int($value, int $min, int $max): int {
  $num = (int) round((float) $value);
  return max($min, min($max, $num));
}

function settings_clamp_float($value, float $min, float $max, int $precision = 2): float {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $payload['ultrasonic']['collection_threshold_pct'] = settings_clamp_int($_POST['ultrasonic_collection_threshold'] ?? $payload['ultrasonic']['collection_threshold_pct'], 0, 100);
  $payload['ultrasonic']['alert_pct'] = settings_clamp_int($_POST['ultrasonic_alert_pct'] ?? $payload['ultrasonic']['alert_pct'], 0, 100);
  $payload['ultrasonic']['critical_pct'] = settings_clamp_int($_POST['ultrasonic_critical_pct'] ?? $payload['ultrasonic']['critical_pct'], 0, 100);
  $payload['ultrasonic']['empty_distance_cm'] = settings_clamp_float($_POST['ultrasonic_empty_cm'] ?? $payload['ultrasonic']['empty_distance_cm'], 0, 400, 1);
  $payload['ultrasonic']['full_distance_cm'] = settings_clamp_float($_POST['ultrasonic_full_cm'] ?? $payload['ultrasonic']['full_distance_cm'], 0, 400, 1);
  $payload['ultrasonic']['poll_interval_sec'] = settings_clamp_int($_POST['ultrasonic_poll_interval'] ?? $payload['ultrasonic']['poll_interval_sec'], 1, 300);
  $payload['ultrasonic']['smoothing_samples'] = settings_clamp_int($_POST['ultrasonic_smoothing'] ?? $payload['ultrasonic']['smoothing_samples'], 1, 25);
  $payload['ultrasonic']['offline_timeout_sec'] = settings_clamp_int($_POST['ultrasonic_offline_timeout'] ?? $payload['ultrasonic']['offline_timeout_sec'], 5, 900);

  $payload['camera']['model'] = 'YOLOv5';
  $payload['camera']['min_confidence'] = settings_clamp_float($_POST['camera_min_confidence'] ?? $payload['camera']['min_confidence'], 0.1, 1.0, 2);
  $payload['camera']['nms_iou'] = settings_clamp_float($_POST['camera_nms_iou'] ?? $payload['camera']['nms_iou'], 0.1, 1.0, 2);
  $payload['camera']['frame_interval_sec'] = settings_clamp_float($_POST['camera_frame_interval'] ?? $payload['camera']['frame_interval_sec'], 0.1, 10.0, 2);
  $payload['camera']['min_consecutive_hits'] = settings_clamp_int($_POST['camera_min_hits'] ?? $payload['camera']['min_consecutive_hits'], 1, 10);
  $payload['camera']['exposure_compensation'] = settings_clamp_float($_POST['camera_exposure'] ?? $payload['camera']['exposure_compensation'], -2.0, 2.0, 1);

  $payload['gps']['module'] = 'NEO-6M';
  $payload['gps']['update_rate_hz'] = settings_clamp_int($_POST['gps_update_rate'] ?? $payload['gps']['update_rate_hz'], 1, 10);
  $payload['gps']['min_satellites'] = settings_clamp_int($_POST['gps_min_sat'] ?? $payload['gps']['min_satellites'], 3, 12);
  $payload['gps']['hdop_max'] = settings_clamp_float($_POST['gps_hdop_max'] ?? $payload['gps']['hdop_max'], 0.5, 10.0, 2);
  $payload['gps']['drift_correction_m'] = settings_clamp_int($_POST['gps_drift_correction'] ?? $payload['gps']['drift_correction_m'], 0, 50);
  $payload['gps']['offset_lat'] = settings_clamp_float($_POST['gps_offset_lat'] ?? $payload['gps']['offset_lat'], -1.0, 1.0, 6);
  $payload['gps']['offset_lng'] = settings_clamp_float($_POST['gps_offset_lng'] ?? $payload['gps']['offset_lng'], -1.0, 1.0, 6);
  $payload['gps']['heartbeat_seconds'] = settings_clamp_int($_POST['gps_heartbeat'] ?? $payload['gps']['heartbeat_seconds'], 5, 300);
...
render_header('Settings', ['active' => 'settings', 'brand' => APP_BRAND_FULL]);
...
<form method="post" class="row g-3">
  ...
  <div class="col-12 col-xl-4">
    <div class="dashboard-panel p-4 h-100">
      ...
      <p class="small text-secondary mb-2">Model: YOLOv5 (trained with OpenCV). Tune detection thresholds to match your bin environment.</p>
      <div class="row g-2">

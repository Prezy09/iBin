<?php
// includes/layout.php
// Shared layout helpers. Usage: include_once __DIR__.'/layout.php'; render_header('Title'); ... render_footer();
...
if (!function_exists('render_header')) {
  function render_header(string $title = 'iBin Dashboard', array $opts = []) {
    $active = $opts['active'] ?? '';
    ...
    $brand_attr = htmlspecialchars($brand, ENT_QUOTES);
    $theme_attr = htmlspecialchars($theme, ENT_QUOTES);
    $basiKey = $opts['basi_api_key'] ?? getenv('BASI_API_KEY') ?? getenv('BASIC_API_KEY') ?? 'eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6IjFhMzQ1OWY2YWU0OTQwZTk5M2JkNzhiNWUxZWI0NzcxIiwiaCI6Im11cm11cjY0In0=';
    ...
    <aside class="sidebar border-end bg-white">
      ...
      <div class="sidebar-content d-flex flex-column">
        <nav class="nav flex-column app-nav flex-grow-1">
          <a class="nav-link <?= $active === 'dashboard' ? 'active' : '' ?>" href="<?= $baseHref ?>dashboard.php">
            <i class="fa-solid fa-gauge-high nav-icon"></i>
            <span>Dashboard</span>
          </a>
          <?php if ($role === 'Admin' || $role === 'Operator'): ?>
            <a class="nav-link <?= $active === 'bins' ? 'active' : '' ?>" href="<?= $baseHref ?>bins.php">
              <i class="fa-solid fa-trash-can nav-icon"></i>
              <span>Bins</span>
            </a>
            <a class="nav-link <?= $active === 'collection' ? 'active' : '' ?>" href="<?= $baseHref ?>collection.php">
        ...
        <div class="sidebar-footer p-3 border-top">
          <a class="btn btn-outline-danger w-100 d-flex align-items-center justify-content-center gap-2" href="<?= $baseHref ?>logout.php">
...
if (!function_exists('render_footer')) {
  function render_footer() {
    $env = $GLOBALS['_layout_env'] ?? ['asset_prefix' => '/assets'];
    $assetPrefix = $env['asset_prefix'] ?? '/assets';
